...
public class Item implements GamePiece {
    ...
    public static Item generateAttackItem() throws FileNotFoundException {
        File txt = new File("src/main/java/textfiles/AttackItems");
        int n = 0;
        int lowerBound = 0;
        int upperBound = 0;
    ...
    public static Item generateDefenseItem() throws FileNotFoundException {
        File txt = new File("src/main/java/textfiles/DefenseItems");
        int n = 0;
        int lowerBound = 0;
        int upperBound = 0;
        String itemName = null;
    ...
    public static Item generateHealthItem() throws FileNotFoundException {
        File txt = new File("src/main/java/textfiles/HealthItems");
        int n = 0;
        int lowerBound = 0;
        int upperBound = 0;
        String itemName = null;
    ...
    public static ArrayList<Item> getItemsHere(int[] position) {
        ArrayList<Item> itemsHere = new ArrayList<>();
        for(Item i: items) {
            if(Arrays.toString(i.getPosition()).equals(Arrays.toString(position))) itemsHere.add(i);
        }
        return itemsHere;
    }
    ...
    public String toString() {
        String e = this.isEquipped() ? "[EQPD]" : "";
        StringBuilder sb = new StringBuilder();
        sb.append("%s %s %s".formatted(this.getIcon(), this.getName(), e));
        sb.append(this.dmgString());
        sb.append(this.defString());
        sb.append(this.hlthString());
        return sb.toString();
    }

    private String dmgString() {
        return this.getDamageEffect() > 0 ? "\n+%s Damage".formatted(this.getDamageEffect()) : "";
    }

    private String defString() {
        return this.getDefenseEffect() > 0 ? "\n+%s Defense".formatted(this.getDefenseEffect()) : "";

    }

    private String hlthString() {
        return this.getHealthEffect() > 0 ? "\n+%s Health".formatted(this.getHealthEffect()) : "";
    }
