...
public class Goblin implements Player, GamePiece {
    ...
    private static String generateName() throws FileNotFoundException {
        String name = "Goblin";
        File txt = new File("src/Java/GoblinNames");
        int n = 0;
    ...
    public static Goblin getGoblin(int id) {
        for(Goblin g: goblins) if(g.getId() == id) return g;
        return null;
    }
    ...
    public static boolean notAllDead() {
        for(Goblin g: goblins) if(g.isAlive()) return true;
        return false;
    }
    ...
    public String toString() {
        return String.format("%s\n", this.getIcon()+this.getName() + " (H%s)".formatted(this.getId()))
                ...
                + String.format("%s\n", "Mental Strength: " + this.getAggression())
                + String.format("%s\n", "Position: " + Arrays.toString(this.getPosition()));
    }

    public static void moveGoblin(int id, ArrayList<Human> humans, ArrayList<Item> items) {
        Goblin g = getGoblin(id);
        ...
        pickUpItemsHere(g, items);
        g.healGoblinsHere();
        fightTheHumansHere(g, humans, items);
    }
    ...
    private static void pickUpItemsHere(Goblin g, ArrayList<Item> items) {
        ArrayList<Item> itemsToRemove = new ArrayList<>();
        for(Item i: items) {
            if(Arrays.toString(i.getPosition()).equals(Arrays.toString(g.getPosition()))) {
                g.setInventory(i);
                itemsToRemove.add(i);
            }
        }
    ...
    private static void fightTheHumansHere(Goblin g, ArrayList<Human> humans, ArrayList<Item> items) {
        ArrayList<Human> humansHere = g.getHumansHere(humans);
        switch(humansHere.size()) {
            case 0: break;
            case 1: {
                g.fightHuman(humansHere.get(0), items, Math.random());
                break;
            }
            default: {
                g.ambushHumans(humansHere);
                break;
            }
        }
    }

    private ArrayList<Human> getHumansHere(ArrayList<Human> humans) {
    ...
    public void isDead(ArrayList<Item> items) {
        ...
        this.setAggression(-this.getAggression());
